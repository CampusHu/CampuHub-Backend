...
public class ExamController {
    ...
    public SuccessResponse<List<ExamFindAllResponse>> findUserExamByCourse(@Login LoginUser loginUser,@RequestParam Long userCourseId) {
        return SuccessResponse.<List<ExamFindAllResponse>>builder()
                .status(200)
                .message("강의별 학생 조회 성공")
                .data(examService.getExamScores(loginUser,userCourseId))
                .build();
    }
       //  강의별 학생 성적 기입
    @PostMapping("/api/professor/exam/condition")
    @ResponseStatus(HttpStatus.OK)
    public SuccessResponse<Void> createExams(@Login LoginUser loginUser, @RequestBody ExamScoreInputRequest examRequest) {
        examService.updateExamScore(loginUser,examRequest);
